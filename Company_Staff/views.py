...
def recurring_invoice(request):
    if 'login_id' in request.session:
        ...
        if log_details.user_type == 'Staff':
                ...
                return render(request,'zohomodules/recurring_invoice/recurring_invoice_listout.html',content)
        if log_details.user_type == 'Company':
            dash_details = CompanyDetails.objects.get(login_details=log_details)
            item=Items.objects.filter(company=dash_details)
            allmodules= ZohoModules.objects.get(company=dash_details,status='New')
        ...
        return render(request,'zohomodules/recurring_invoice/recurring_invoice_listout.html',context)
...
def recurring_invoice_create(request):
    ...
    return render(request,'zohomodules/recurring_invoice/recurring_invoice_create.html',{'customers':customers})





def recurringinvoice_customer(request):
    if 'login_id' in request.session:
        if request.session.has_key('login_id'):
            log_id = request.session['login_id']
        ...
        return render(request,'zohomodules/recurring_invoice/create_customer_recurring.html',{'details':dash_details,'allmodules': allmodules,'comp_payment_terms':comp_payment_terms,'log_details':log_details,'price_lists':price_lists}) 
    else:
        return redirect('/')  
